...
def investor_statistics(request):
    ...
    try:
        ...
        active_investments = investments.filter(
            business__current_funding__lt=F('business__funding_goal')
        ).count()
        # Get completed investments (fully funded businesses)
        completed_investments = investments.filter(
            business__current_funding__gte=F('business__funding_goal')
        ).count()
        ...
        for distribution in profit_distributions:
            profit_history.append({
                ...
                'log_month: distribution.log.get_month_display() if distribution.log.month else "Unknown Month",
                'log_year: distribution.log.year if distribution.log.year else "Unknown Year"
            })
        ...
        monthly_profit_data = ProfitDistribution.objects.filter(user=user).values(
            ...
            total_profit=Sum('amount_distributed')
        ...
        for investment in investments.select_related('business'):
            ...
            ).aggregate(total=Sum('amount_distributed'))['total'] or 0
        ...
        monthwise = {}
        for log in logs:
            key = f"{log.year or 0}-{log.month or 0:02d}" if log.month and log.year else "Unknown"
